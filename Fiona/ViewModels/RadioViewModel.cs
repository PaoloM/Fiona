    ...
    public class RadioViewModel : BaseViewModel
    {
        ...
        public Applet CurrentApplet
        {
            ...
            set
            {
                ...
                {
                    AppletTitle = ResourceExtensions.GetLocalized("Shell_Radio.Content");
                    PlaylistCommandsVisibility = Visibility.Collapsed;
                    TitleVisibility = Visibility.Collapsed;
                    Apps = FionaDataService.GetAllRadios(FionaDataService.CurrentPlayer);
                }
                else
                {
                    ...
                    if (value.GetID == null)
                    { // app top level
                        Apps = FionaDataService.GetRadioTopLevel(FionaDataService.CurrentPlayer, value.GetMenu);
        ...
        private void AppSelected(Applet applet)
        {
            ...
            if (applet.GoAction is null)
            {
                if (applet?.Style != "itemNoAction")
                {
                    ...
                    {
                        NavigationService.Navigate<RadioPage>(applet);
                    }
                }
            }
        }
